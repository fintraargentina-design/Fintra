...
const fetchIndustries = async (
  ...
  if (!rpcError && rpcData) {
    ...
    // Agregar opción "All Industries"
    if (options.length > 0) {
      const total = options.reduce((sum: number, opt: { count: number }) => sum + opt.count, 0);
      return [
        { 
          value: 'All Industries', // Changed from 'Todas' to match English convention, SectorAnalysisPanel supports both
          label: `All Industries (${total})`, 
          count: total 
        },
        ...options
      ];
  ...
  if (options.length > 0) {
    const total = options.reduce((sum, opt) => sum + opt.count, 0);
    return [
      { value: 'All Industries', label: `All Industries (${total})`, count: total },
      ...options
    ];
  }
