-- Migration: Optimize calculate_ticker_performance to avoid timeouts
-- 1. Remove redundant index
-- 2. Limit historical scan to ~5 years window

DROP INDEX IF EXISTS public.prices_daily_ticker_date_idx;
...
DECLARE
    ...
    -- Optimization: Max Window Date
    -- We restrict the historical scan to ~6 years to cover the 5Y trading window (1260 days)
    -- This prevents scanning full history (e.g. 20+ years) which causes timeouts.
    v_min_date date;
...
BEGIN
    ...
    -- Set Cutoff Date for optimization (5 Years + Buffer)
    v_min_date := v_latest_date - interval '5 years' - interval '2 months';
    ...
    WITH price_history AS (
        ...
        FROM prices_daily
        WHERE ticker = p_ticker
          AND price_date >= v_min_date -- OPTIMIZATION: Date Filter
    ),
    ytd_base AS (
        SELECT adj_close
        FROM prices_daily
        WHERE ticker = p_ticker 
          AND price_date < date_trunc('year', v_latest_date)
          AND price_date >= v_min_date -- Keep consistent
        ORDER BY price_date DESC
        LIMIT 1
    )
    SELECT
    ...
    WITH relevant_history AS (
        ...
        WHERE ticker = p_ticker
          AND price_date >= v_min_date -- OPTIMIZATION: Date Filter
        ORDER BY price_date DESC
        LIMIT 1261 -- Max window (5Y) + 1 (Redundant with date filter but safe)
    ),
    daily_returns AS (
        SELECT 
            rn,
            ...
            (
                WITH subset AS (
                    ...
                    WHERE d.ticker = p_ticker 
                      AND d.price_date >= date_trunc('year', v_latest_date)
                      AND d.price_date >= v_min_date -- Keep consistent
                ),
                dd_series AS (
