...
export async function recomputeFGOSForTicker(ticker: string, snapshotDate?: string) {
  ...
  const sector = snap.sector || (snap as any).profile_structural?.classification?.sector || null;
  const benchmarks = sector ? await getBenchmarksForSector(sector) : null;
  if (!benchmarks) {
    await supabaseAdmin
      .from('fintra_snapshots')
  ...
  const ratios = (ratiosRes.status === 'fulfilled' ? ratiosRes.value?.[0] : {}) as FmpRatios;
  const metrics = (metricsRes.status === 'fulfilled' ? metricsRes.value?.[0] : {}) as FmpMetrics;
  const snapshotRow = {
    ticker: ticker,
    snapshot_date: date,
    sector: sector,
