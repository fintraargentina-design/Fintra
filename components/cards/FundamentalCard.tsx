...
export default function FundamentalCard({ symbol, scrollRef }: { symbol: string; scrollRef?: React.RefObject<HTMLDivElement> }) {
            ...
            <TableRow className="border-zinc-800 hover:bg-[#1D1D1D] bg-[#1D1D1D] border-b-0">
              ...
              {data?.years.map(year => (
                year.columns.map(col => (
                    <TableHead key={col} className="px-2 text-gray-300 text-[10px] h-6 text-center whitespace-nowrap">
                        {col}
                    </TableHead>
                ))
              ))}
