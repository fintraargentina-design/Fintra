...
async function main() {
  ...
  const { runIndustryClassificationSync } =
    await import("@/app/api/cron/industry-classification-sync/core");
  const { runPricesDailyBulk } =
    await import("@/app/api/cron/prices-daily-bulk/core");
  const { runFinancialsBulk } =
    await import("@/app/api/cron/financials-bulk/core");
  const { runCompanyProfileBulk } =
    await import("@/app/api/cron/company-profile-bulk/core");
  const { runIndustryPerformanceAggregator } =
    await import("@/app/api/cron/industry-performance-aggregator/core");
  const { runSectorPerformanceAggregator } =
    await import("@/app/api/cron/sector-performance-aggregator/core");
  const { runSectorPerformanceWindowsAggregator } =
    await import("@/app/api/cron/sector-performance-windows-aggregator/core");
  const { runIndustryPerformanceWindowsAggregator } =
    await import("@/app/api/cron/industry-performance-windows-aggregator/core");
  const { runSectorPeAggregator } =
    await import("@/app/api/cron/sector-pe-aggregator/core");
  const { runIndustryPeAggregator } =
    await import("@/app/api/cron/industry-pe-aggregator/core");
  const { runSectorBenchmarks } =
    await import("@/app/api/cron/sector-benchmarks/core");
  const { runPerformanceBulk } =
    await import("@/app/api/cron/performance-bulk/core");
  const { runMarketStateBulk } =
    await import("@/app/api/cron/market-state-bulk/core");
  const { runDividendsBulkV2 } =
    await import("@/app/api/cron/dividends-bulk-v2/core");
  const { runFmpBulk } = await import("@/app/api/cron/fmp-bulk/core");
  const { checkSnapshotsHealth } =
    await import("@/app/api/cron/healthcheck-fmp-bulk/core");
  ...
  console.log(
    `🚀 [Script] Starting Master Cron (ALL) with LIMIT=${LIMIT === 0 ? "ALL" : LIMIT}...`,
  );
  ...
  try {
    ...
    console.log("\n--- 4b. TTM Valuation Incremental ---");
    const { incrementalTTMValuation } =
      await import("@/scripts/pipeline/04b-incremental-ttm-valuation");
    ...
    // 6. Sector Performance Aggregator (1D) - MOVED UP
    console.log("\n--- 6. Sector Performance Aggregator (1D) ---");
    await runSectorPerformanceAggregator();

    // 7. Industry Performance Aggregator (1D) - MOVED DOWN
    console.log("\n--- 7. Industry Performance Aggregator (1D) ---");
    await runIndustryPerformanceAggregator();

    // 8. Industry Performance Windows Aggregator - MOVED UP
    console.log("\n--- 8. Industry Performance Windows Aggregator ---");
    await runIndustryPerformanceWindowsAggregator();

    // 9. Sector Performance Windows Aggregator - MOVED DOWN
    console.log("\n--- 9. Sector Performance Windows Aggregator ---");
    ...
    const { runIndustryBenchmarksAggregator } =
      await import("@/app/api/cron/industry-benchmarks-aggregator/core");
    ...
    const { runPerformanceWindowsAggregator } =
      await import("@/app/api/cron/performance-windows-aggregator/core");
    ...
    const { recomputeFGOSForTicker } =
      await import("@/lib/engine/fgos-recompute");
    ...
    if (allTickers.length > 0) {
      const { runRecomputeFGOSBulk } =
        await import("@/app/api/cron/recompute-fgos-bulk/core");
