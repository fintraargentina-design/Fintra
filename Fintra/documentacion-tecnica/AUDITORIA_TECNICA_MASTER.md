...
Medida relativa de posición competitiva (v1.2).
*   **Estados:** `Leader` (Positivo), `Follower` (Neutro), `Laggard` (Negativo).
*   **Base:** Comparación estricta contra pares de la misma Industria (no Sector).
*   **Ventanas Auditadas:** 1M, 3M, 6M, 1Y, 2Y, 3Y, 5Y. (Se excluyen explícitamente YTD y 1W por ruido).
*   **Lógica de Bloques:**
    *   Short: 1M, 3M
    *   Mid: 6M, 1Y, 2Y
    *   Long: 3Y, 5Y
*   **Mecanismo:** Votación por mayoría de bloques (Block Voting). Requiere al menos 2 bloques válidos.
