...
import type { FMPFinancialRatio } from "@/lib/fmp/types";
...
import {
  ...
  calculateDimensionalConfidence,
...
function calculateComponent(
  ...
  const score =
    results.reduce((sum, r) => sum + r.effective, 0) / results.length;
...
export function computeFGOS(
  ...
  const profileOk =
    snapshot.profile_structural &&
    (snapshot as any).profile_structural.status !== "pending";
  if (!profileOk) {
    return {
      fgos_score: null,
  ...
  const hasLowConfidence = Object.values(benchmarks).some(
    (b) => b && typeof b === "object" && (b as any).confidence === "low",
  );
  const growthResult = calculateComponent([
    { value: growth.revenue_cagr, benchmark: (benchmarks as any).revenue_cagr },
    {
      value: growth.earnings_cagr,
      benchmark: (benchmarks as any).earnings_cagr,
    },
  ...
  const profitabilityResult = calculateComponent([
    { value: metrics?.roicTTM, benchmark: (benchmarks as any).roic },
    {
      ...
      benchmark: (benchmarks as any).operating_margin,
    },
    {
      value: ratios?.netProfitMarginTTM,
      benchmark: (benchmarks as any).net_margin,
    },
  ]);
  const efficiencyResult = calculateComponent([
    { value: metrics?.roicTTM, benchmark: (benchmarks as any).roic },
    {
      value: metrics?.freeCashFlowMarginTTM,
      benchmark: (benchmarks as any).fcf_margin,
    },
  ]);
  ...
  const solvencyResult = calculateComponent([
    {
      ...
      benchmark: (benchmarks as any).debt_to_equity,
    },
    {
      value: ratios?.interestCoverageTTM,
      benchmark: (benchmarks as any).interest_coverage,
    },
  ]);
  const WEIGHTS = {
    growth: 0.25,
  ...
  let competitiveAdvantageResult: ReturnType<
    typeof calculateCompetitiveAdvantage
  > | null = null;
  if (history && Array.isArray(history) && history.length > 0) {
    const caHistory: CompetitiveAdvantageHistoryRow[] = (history as any[]).map(
      (row) => ({
  ...
  if (validComponents.length > 0) {
    const totalWeightedScore = validComponents.reduce(
...
export async function recomputeFGOSForTicker(
  ...
  const sector =
    (snap as any).sector ||
    (snap as any).profile_structural?.classification?.sector ||
    null;
  ...
  const { data: financials } = await supabaseAdmin
    ...
    .select(
      "roic, operating_margin, net_margin, fcf_margin, debt_to_equity, interest_coverage, period_type, period_label, period_end_date",
    )
    .eq("ticker", ticker)
  ...
  const ratios: FMPFinancialRatio = {
    symbol: ticker,
    date: date,
    period: "TTM",
  ...
  const metrics: any = {
    ...
    piotroskiScore: (snap as any).profile_structural?.financial_scores
      ?.piotroski_score,
  };
  ...
  const { data: history } = await supabaseAdmin
    .from("datos_financieros")
    .select(
      "period_end_date, period_type, revenue, roic, roe, gross_margin, operating_margin, net_margin, invested_capital, free_cash_flow, capex, weighted_shares_out",
    )
    .eq("ticker", ticker)
    .eq("period_type", "FY")
  ...
  const { data: valuationsData } = await supabaseAdmin
    ...
    .select(
      "valuation_date, denominator_type, pe_ratio, ev_ebitda, price_to_fcf, price_to_sales",
    )
    .eq("ticker", ticker)
  ...
  if (history && history.length >= 3) {
    ...
    if (revenueGrowth.length >= 2) {
      const mean =
        revenueGrowth.reduce((a, b) => a + b, 0) / revenueGrowth.length;
      const variance =
        revenueGrowth.reduce((a, b) => a + Math.pow(b - mean, 2), 0) /
  ...
  const snapshotRow = {
    ...
    engine_version: "v2", // Required by type
  ...
  return {
    status: result.fgos_score !== null ? "computed" : "pending",
    score: result.fgos_score,
  };
}
