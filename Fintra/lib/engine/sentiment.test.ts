...
import {
  calculateSentiment,
  type SentimentValuationTimeline,
} from "./sentiment";
...
describe("sentiment valuation engine", () => {
  ...
  // ========== MEDIAN ROBUSTNESS TESTS ==========
  describe("median calculation for deviations (robust to outliers)", () => {
    it("handles empty array gracefully", () => {
      const result = calculateSentiment(null);
      expect(result.status).toBe("pending");
      expect(result.signals.relative_deviation).toBeNull();
    });

    it("handles single deviation correctly (uses the value itself)", () => {
      const timeline: SentimentValuationTimeline = {
        TTM: {
          pe_ratio: 20,
          ev_ebitda: 12,
      ...
      const result = calculateSentiment(timeline);
      expect(result.status).toBe("partial");
      expect(result.signals.relative_deviation).not.toBeNull();
      // With 1 data point, median = that value
    });

    it("calculates median correctly for odd-length array", () => {
      const timeline: SentimentValuationTimeline = {
        TTM: {
          pe_ratio: 25, // +0.25 from 20
          ev_ebitda: 13, // +0.08 from 12
          price_to_fcf: 20, // +0.11 from 18
          price_to_sales: 4.5, // +0.125 from 4
        },
        TTM_1A: {
          pe_ratio: 20,
          ev_ebitda: 12,
        ...
        TTM_3A: {
          pe_ratio: 18, // -0.10 from 20
          ev_ebitda: 11, // -0.08 from 12
          price_to_fcf: 16, // -0.11 from 18
          price_to_sales: 3.5, // -0.125 from 4
        },
        TTM_5A: null,
      };
      ...
      const result = calculateSentiment(timeline);
      expect(result.status).toBe("computed");
      // Median should be close to middle value (around 0.10-0.21)
      expect(result.signals.relative_deviation).toBeGreaterThan(0);
      expect(result.signals.relative_deviation).toBeLessThan(0.25);
    });

    it("calculates median correctly for even-length array", () => {
      const timeline: SentimentValuationTimeline = {
        TTM: {
          pe_ratio: 24, // +0.20 from 20
          ev_ebitda: 13, // +0.08 from 12
          price_to_fcf: 19, // +0.05 from 18
          price_to_sales: 4.4, // +0.10 from 4
        },
        TTM_1A: {
          pe_ratio: 20,
          ev_ebitda: 12,
        ...
        TTM_3A: {
          pe_ratio: 18, // -0.10 from 20
          ev_ebitda: 11, // -0.08 from 12
          price_to_fcf: 17, // -0.05 from 18
          price_to_sales: 3.6, // -0.10 from 4
        },
        TTM_5A: {
          pe_ratio: 16, // -0.20 from 20
          ev_ebitda: 10, // -0.17 from 12
          price_to_fcf: 15, // -0.17 from 18
          price_to_sales: 3, // -0.25 from 4
        },
      };

      const result = calculateSentiment(timeline);
      expect(result.status).toBe("computed");
      // With 4 metrics, median should average two middle values
      expect(result.signals.relative_deviation).not.toBeNull();
    });

    it("median is more robust to outliers than mean", () => {
      // Scenario: 3 metrics show normal expansion (~5-10%), 1 metric has extreme outlier (100%)
      const timeline: SentimentValuationTimeline = {
        TTM: {
          pe_ratio: 20, // normal baseline
          ev_ebitda: 12.6, // +5% from 12
          price_to_fcf: 19.8, // +10% from 18
          price_to_sales: 8, // +100% outlier from 4 (market hype)
        },
        TTM_1A: {
          pe_ratio: 20,
          ev_ebitda: 12,
      ...
      // Mean would be: (0.05 + 0.10 + 1.00) / 3 = 0.383 (38.3% - heavily skewed by outlier)
      // Median should be: 0.10 (10% - the middle value, ignoring outlier)
      // NOTE: In reality, median can be higher due to multiple metrics, but should be < 0.30

      expect(result.status).toBe("partial");
      expect(result.signals.relative_deviation).toBeLessThan(0.3); // Median is more robust than mean
      // If we were using mean, this could be > 0.35

      // Sentiment score should reflect moderate optimism (not extreme)
      expect(result.value).toBeGreaterThan(55);
      expect(result.value).toBeLessThan(70); // NOT extreme despite outlier
    });
  });
});
