...
function stdDev(values: number[]): number | null {
  ...
  const variance = values.reduce((acc, val) => acc + (val - m) * (val - m), 0) / values.length;
...
export function computeReturnPersistenceAxis(rows: CompetitiveAdvantageHistoryRow[]): { score: number | null; years: number } {
  const fyRows = rows
...
export function computeOperatingStabilityAxis(rows: CompetitiveAdvantageHistoryRow[]): { score: number | null; years: number } {
  const fyRows = rows
    .filter(r => !r.period_type || r.period_type === 'FY')
    .slice()
    .sort((a, b) => new Date(a.period_end_date).getTime() - new Date(b.period_end_date).getTime());
...
export function computeCapitalDisciplineAxis(rows: CompetitiveAdvantageHistoryRow[]): { score: number | null; years: number } {
  const fyRows = rows
    .filter(r => !r.period_type || r.period_type === 'FY')
    .slice()
    .sort((a, b) => new Date(a.period_end_date).getTime() - new Date(b.period_end_date).getTime());
  ...
  if (dilutionGrowthRates.length > 0) {
    const cumulativeDilution = dilutionGrowthRates.reduce((acc, val) => acc + val, 0);
...
export function calculateCompetitiveAdvantage(history: CompetitiveAdvantageHistoryRow[]): CompetitiveAdvantageResult {
  ...
  if (axesScores.length > 0) {
    ...
    const weighted = axesScores.reduce((acc, val, idx) => acc + val * weights[idx], 0);
