...
export function deriveFinancialMetrics(params: {
  ...
  if (
    ...
    console.log(
      "[DEBUG deriveFinancialMetrics] balance keys:",
      Object.keys(balance).slice(0, 15),
    );
    console.log(
      "[DEBUG deriveFinancialMetrics] Looking for cash with quoted key:",
      balance['"cashAndCashEquivalents"'],
    );
  }
  ...
  const wacc =
    periodType === "TTM" && metricsTTM?.wacc ? metricsTTM.wacc : null;
  ...
  const daysSinceEnd =
    (Date.now() - new Date(periodEndDate).getTime()) / (1000 * 60 * 60 * 24);
  ...
  if (historicalIncome && historicalIncome.length >= 3) {
    // Build value arrays
    const revs = historicalIncome.map((r) => ({
      date: r.date,
      value: r.revenue,
    }));
    const earns = historicalIncome.map((r) => ({
      date: r.date,
      value: r.netIncome,
    }));
  ...
  if (historicalBalance && historicalBalance.length >= 3) {
    const eqs = historicalBalance.map((r) => ({
      date: r.date,
      value: r.totalStockholdersEquity,
    }));
