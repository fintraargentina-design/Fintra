...
export async function backfillSnapshotsForDate(
  date: string,
  targetTicker?: string,
) {
  ...
  for (const ticker of uniqueTickers) {
    ...
    if (!fin) {
      if (targetTicker)
        console.warn(`[Backfill] No financials found for ${ticker} on ${date}`);
      continue;
    }
    ...
    if (!sector) {
      if (targetTicker)
        console.warn(`[Backfill] No sector found for ${ticker}`);
      continue;
    }
    ...
    try {
      ...
      console.error(`[${fin.ticker}] FGOS CALCULATION FAILED:`, error.message);
    ...
    const { error } = await supabase
      ...
      .upsert(snapshot, { onConflict: "ticker,snapshot_date" });
    if (error) {
      console.error(
        `[Backfill] Error upserting snapshot for ${ticker}:`,
        error,
      );
    } else {
      if (targetTicker)
        console.log(`[Backfill] Snapshot upserted for ${ticker}`);
    }
  }
}
