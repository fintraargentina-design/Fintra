...
## Supabase Query Limitations (CRITICAL)

**Rule:** Supabase JS client silently truncates results to 1000 rows by default.
...
```typescript
// ✅ CORRECT - Safe Chunk Size
const CHUNK_SIZE = 100; // 100 * 7 windows = 700 rows (< 1000 limit)
for (let i = 0; i < tickers.length; i += CHUNK_SIZE) {
  const chunk = tickers.slice(i, i + CHUNK_SIZE);
  // fetch...
}

// ❌ WRONG - Exceeds 1000 row limit silently
const CHUNK_SIZE = 1000; // 1000 * 7 windows = 7000 rows -> Truncated to 1000!
// Only ~140 tickers will be processed, rest ignored silently
```
...
## Documentation & User Messaging Philosophy
...
3. Every alert or message must:
   a) Describe an analytical scenario
   b) Explain what that scenario means
   c) Explicitly suggest what dimensions the analyst should observe
4. Language must be descriptive, neutral, and operational.
5. Replace "risk", "warning", or "verdict" framing with "scenario" and "analytical focus".

**Action:** If a section cannot be rewritten following this framework, stop and flag it.
