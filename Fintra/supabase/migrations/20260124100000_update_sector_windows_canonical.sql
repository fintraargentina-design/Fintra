-- Update Sector Aggregation Function to use Canonical Windows (Phase 9)
-- Removes 1W, YTD. Adds 3M, 6M, 2Y. Keeps 1M, 1Y, 3Y, 5Y.
...
BEGIN
    ...
    unpivoted AS (
        ...
        UNION ALL
        SELECT sector, '3M' as window_code, ret_3m as return_percent FROM aggregated WHERE ret_3m IS NOT NULL
        UNION ALL
        SELECT sector, '6M' as window_code, ret_6m as return_percent FROM aggregated WHERE ret_6m IS NOT NULL
        UNION ALL
        SELECT sector, '1Y' as window_code, ret_1y as return_percent FROM aggregated WHERE ret_1y IS NOT NULL
        UNION ALL
        SELECT sector, '2Y' as window_code, ret_2y as return_percent FROM aggregated WHERE ret_2y IS NOT NULL
        UNION ALL
