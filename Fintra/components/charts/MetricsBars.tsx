...
import { TooltipComponent, GridComponent, TitleComponent, DatasetComponent, ToolboxComponent } from "echarts/components";
import { CanvasRenderer } from "echarts/renderers";
import type { RawMetrics } from "@/lib/metricsNormalize";
import { normalizeMetrics } from "@/lib/metricsNormalize";

echarts.use([BarChart, TooltipComponent, GridComponent, TitleComponent, DatasetComponent, ToolboxComponent, CanvasRenderer]);
const ReactECharts = dynamic(() => import("echarts-for-react/lib/core"), { ssr: false });

export default function MetricsBars({ raw, title = "Métricas normalizadas (0–100)", symbol }:{
  raw: RawMetrics;
  title?: string;
  symbol?: string;
}) {
    ...
    toolbox: {
      feature: {
        saveAsImage: {
          ...
          name: symbol ? `${symbol} - Metrics` : "Metrics",
