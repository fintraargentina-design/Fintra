...
import { Globe, RefreshCw, Zap, AlertTriangle } from "lucide-react";
...
export default function EcosystemCard({ 
  ...
  if (!loading && !hasData) {
    return (
      <Card className="bg-[#0e0e0e] border border-[#222] shadow-sm h-full flex flex-col items-center justify-center p-6 text-gray-500 relative">
        <Button 
            variant="ghost" 
            size="sm" 
            onClick={loadMock} 
        ...
        <Button onClick={() => fetchEcosystemData(true)} variant="outline" className="border-zinc-700 text-zinc-400 hover:bg-[#111] hover:text-zinc-200">
  ...
  return (
    <Card className="bg-[#0e0e0e] border border-[#222] shadow-none h-full flex flex-col group relative overflow-hidden rounded-none">
      {/* Loading Overlay */}
      {loading && hasData && (
        <div className="absolute inset-0 bg-black/60 backdrop-blur-[1px] z-50 flex flex-col items-center justify-center text-white/80">
           <FintraLoader size={32} className="mb-2" />
           <span className="text-xs font-medium tracking-wider">ACTUALIZANDO...</span>
        </div>
      )}

      <CardHeader className="p-0 m-0 space-y-0 shrink-0 w-full border-b border-[#222] bg-[#0e0e0e] z-10">
        <div className="flex items-center justify-between w-full h-10 px-3">
          ...
          variant="ghost" 
          size="icon" 
          onClick={() => fetchEcosystemData(true)} 
          disabled={loading}
          className="h-6 w-6 text-zinc-500 hover:text-zinc-200 hover:bg-[#111]"
        >
          {loading ? <FintraLoader size={12} /> : <RefreshCw className="w-3 h-3" />}
        </Button>
          </div>
          ...
          <div className="flex items-center gap-3">
             ...
             <Button variant="ghost" size="sm" onClick={loadMock} className="text-[9px] text-zinc-600 hover:text-zinc-300 hover:bg-[#111] h-5 px-1">
               Mock
             </Button>
          </div>
        </div>
      ...
      <CardContent className="p-0 flex-1 min-h-[350px] relative bg-[#0e0e0e]">
        {loading && !hasData ? (
