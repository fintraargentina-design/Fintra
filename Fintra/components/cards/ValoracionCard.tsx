...
export default function ValoracionCard({ symbol, scrollRef, peerTicker, highlightedMetrics, defaultExpanded = false, hideExpandButton = false }: { symbol: string; scrollRef?: React.RefObject<HTMLDivElement | null>; peerTicker?: string | null; highlightedMetrics?: string[] | null; defaultExpanded?: boolean; hideExpandButton?: boolean }) {
  ...
  useEffect(() => {
    ...
    const fetchPeer = async () => {
        try {
            const res = await fetch(`/api/analysis/valuation-timeline?ticker=${peerTicker}`);
            if (res.ok) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/analysis/valuation-timeline?ticker=${symbol}`);
        if (!res.ok) {
            console.error("Failed to fetch valuation timeline");
            return;
        }
        const json = await res.json();
              ...
              {loading ? (
                 <TableRow>
                   <TableCell colSpan={100} className="text-center py-8 text-xs text-gray-500">
                     <FintraLoader size={16} className="mr-2 inline-block align-middle"/> <span className="align-middle">Cargando valoración...</span>
                   </TableCell>
                 </TableRow>
              ) : (
