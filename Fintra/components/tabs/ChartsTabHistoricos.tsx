...
export default function ChartsTabHistoricos({
  ...
  // Reset FGOS Data when symbol changes to force refetch
  React.useEffect(() => {
    setFgosData(null);
  }, [symbol]);
  ...
  const getOption = React.useMemo(() => {
    ...
    // Common Options
    const currentViewLabel = VIEWS.find(v => v.key === view)?.label || "Chart";
    const commonOptions = {
      backgroundColor: "transparent",
      animation: true,
      grid: { left: '1%', right: '5%', top: '5%', bottom: '10%', containLabel: true, borderColor: '#333' },
      dataZoom: [{ type: 'inside', realtime: true, start: 0, end: 100 }],
      toolbox: {
        feature: {
          saveAsImage: {
            title: "Guardar imagen",
            name: `${symbol} - ${currentViewLabel}`,
  ...
  return (
    <div className="flex flex-col h-full bg-[#0e0e0e] overflow-hidden border-none shadow-none">
      <div className="flex items-center justify-between w-full bg-[#0e0e0e] shrink-0 z-10 px-2 pt-2 pb-2">
        <div className="flex gap-1 bg-[#1A1A1A] p-0.5 rounded-md border border-[#222]">
          {(["1A", "3A", "5A", "MAX"] as RangeKey[]).map((r) => (
            <button
              key={r}
              onClick={() => setRange(r)}
              className={`
                rounded-[4px] px-2.5 py-0.5 text-[10px] transition-all font-medium
                ${
                  range === r
      ...
      <div className="flex-1 min-h-0 flex items-center justify-center w-full bg-[#0e0e0e]">
        {renderChart()}
      </div>
    </div>
  );
