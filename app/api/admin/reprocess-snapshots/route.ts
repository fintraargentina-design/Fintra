...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`   Solvency filter: ${minSolvency ? `>=${minSolvency}` : 'none'}`);
      ...
      const batchPromise = Promise.all(
        batch.map(async (snapshot) => {
          try {
            ...
            if (!financials) {
              return { ticker: snapshot.ticker, status: 'skipped', reason: 'no_financials' };
            }
            ...
            const benchmarks = await getBenchmarksForSector(snapshot.sector);
