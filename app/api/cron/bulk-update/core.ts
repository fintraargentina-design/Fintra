...
export async function runBulkUpdate(targetTicker?: string, limitParam?: number, offsetParam?: number) {
  ...
  try {
    ...
    const allValidProfiles = rawProfiles.filter(p => {
        ...
        if (targetTicker && sym === targetTicker && !valid) {
             console.warn(`⚠️ Target ticker ${targetTicker} filtered out due to invalid data (price: ${price}, sector: ${sector})`);
        }
        return valid;
    });
