...
export async function GET(req: Request) {
  try {
    ...
    // We limit to a reasonable batch size to avoid timeouts if running sequentially
    // In a real production env, this should be paginated or queue-based.
    // For now, we fetch a batch of active tickers.
    const { data: snapshots, error: snapError } = await supabaseAdmin
      ...
      .limit(10000); // Increased limit to allow full processing (Supabase default is 1000)

    if (snapError) throw snapError;
    if (!snapshots || snapshots.length === 0) {
      return NextResponse.json({ message: 'No active tickers found' });
    }

    // Deduplicate tickers
    const tickers = Array.from(new Set(snapshots.map((s) => s.ticker)));
